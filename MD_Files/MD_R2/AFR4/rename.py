...
AFR4_ROOT = r"C:\Users\HP\Desktop\SilicoScientia\AntiFungal-Project3\MD_plots_R1_R2\AFR4"

# top-level folder names (directly under AFR4_ROOT) that use the
# "prefix" rule. Everything else under AFR4_ROOT uses "replace".
...
# these files are left alone (matches "AFR4_R1", "2AFR4_R1",
# "AFR4_R1_2", etc.). Add more labels here as you add more passes.
PROTECTED_LABELS = {"AFR4_R1"}
...
def plan_prefix(files):
    """Keep a file's leading number, then AFR4_R1, then extension.
...
def main() -> None:
    ...
    root_path = Path(AFR4_ROOT)
    if not root_path.exists():
        print(f"WARNING: folder not found: {AFR4_ROOT}")
        return
    ...
    if not top_level_dirs:
        print(f"No subfolders found in {AFR4_ROOT}")
        return
