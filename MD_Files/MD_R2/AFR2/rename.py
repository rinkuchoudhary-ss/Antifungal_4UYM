...
AFR2_ROOT = r"C:\Users\HP\Desktop\SilicoScientia\AntiFungal-Project3\MD_plots_R1_R2\AFR2"

# top-level folder names (directly under AFR2_ROOT) that use the
# "prefix" rule. Everything else under AFR2_ROOT uses "replace".
...
# these files are left alone (matches "AFR2_R1", "2AFR2_R1",
# "AFR2_R1_2", etc.). Add more labels here as you add more passes.
PROTECTED_LABELS = {"AFR2_R1"}
...
def plan_prefix(files):
    """Keep a file's leading number, then AFR2_R1, then extension.
...
def main() -> None:
    ...
    root_path = Path(AFR2_ROOT)
    if not root_path.exists():
        print(f"WARNING: folder not found: {AFR2_ROOT}")
        return
    ...
    if not top_level_dirs:
        print(f"No subfolders found in {AFR2_ROOT}")
        return
