...
VOR_ROOT = r"C:\Users\HP\Desktop\SilicoScientia\AntiFungal-Project3\MD_plots_R1_R2\VOR"

# top-level folder names (directly under VOR_ROOT) that use the
# "prefix" rule. Everything else under VOR_ROOT uses "replace".
...
# these files are left alone (matches "VOR_R1", "2VOR_R1",
# "VOR_R1_2", etc.). Add more labels here as you add more passes.
PROTECTED_LABELS = {"VOR_R1"}
...
def plan_prefix(files):
    """Keep a file's leading number, then VOR_R1, then extension.
...
def main() -> None:
    ...
    root_path = Path(VOR_ROOT)
    if not root_path.exists():
        print(f"WARNING: folder not found: {VOR_ROOT}")
        return
    ...
    if not top_level_dirs:
        print(f"No subfolders found in {VOR_ROOT}")
        return
